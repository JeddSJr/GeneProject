...
```
```{r}
# Perform GSEA, using correlation coefficient instead of logFC

	# Assign the correlation values to geneList_unsorted

geneList_unsorted <- cor_genes_to_sbs1$Corr_to_SBS1

#Extracting entrez_id and matching with my_sets_t2g

#names(geneList_unsorted) <- sapply(strsplit(rownames(cor_genes_to_sbs1), "\\."), function(x) {
  #if (length(x) >= 2) {
    # The Entrez ID is the second part
    #return(as.character(x[2]))
  #} else {
    #return(NA)  # Return NA if no Entrez ID found
  #}
#})
...
hist(my_geneList, main="Histogram of SBS1 Gene-Level Statistics", xlab="Statistic Value")
	...
	duplicated_genes <- my_geneList[duplicated(my_geneList)]
	head(duplicated_genes)
		...
		### prepare MSigDB gene sets: 
			...
			#OPTIONAL: subset to sub-sections if desired
				my_sets <- all_gene_sets %>%
								dplyr::filter(gs_cat %in% c("H", "C2", "C3", "C5", "C7"))
								
				# Or keep all:
				my_sets <- all_gene_sets
			
			# flatten to dataframe
				my_sets_t2g <- my_sets %>% dplyr::distinct(gs_name, entrez_gene) %>% as.data.frame()
			# determine how many sets we have
				length(unique(my_sets_t2g$gs_name))


			
			# OPTIONAL: filter the gene sets 
				# remove any gene that is not part of our own dataset
					sets_noMissing <- my_sets_t2g %>% filter(entrez_gene %in% names(my_geneList))
					length(unique(sets_noMissing$gs_name))
					

				
				# Identify how many genes are in each set, and retain those with 10 to 1000 genes present in our data
					filtered_gsNames <-	sets_noMissing %>% group_by(gs_name) %>%
											summarize(genes_in_set=n()) %>% 
												filter(genes_in_set >=10, genes_in_set <=1000) %>% select(gs_name)
					

				
				# Remove gene sets that have too few or too many genes
					sets_noMissing_Filtered <- sets_noMissing %>% filter(gs_name %in% filtered_gsNames$gs_name)
					length(unique(sets_noMissing_Filtered$gs_name))
			
				# Indicate which set to use
					sets_toUse_t2g <- sets_noMissing_Filtered
				# OTHERWISE without filtering
					sets_toUse_t2g <- my_sets_t2g

			# stats on how many gene sets will be tested
				sets_toUse_t2g %>% group_by(gs_name) %>%
										summarize(
											genes_in_set=n()) %>% filter(genes_in_set >=10, genes_in_set <=1000)
				
				
				#library(dplyr)

# Checking for duplicates within each gene set
duplicated_in_sets <- sets_toUse_t2g %>%
  group_by(gs_name) %>%
...
#Results_gsea <- read.csv("Results_CPTAC_gseaSBS1.csv")
		...
		# Make a summary of GSEA results
			my_gsea_summary <- head(Results_gsea, n=nrow(Results_gsea)) %>% rowwise() %>% mutate(core_size=length(str_split(core_enrichment, "\\/")[[1]])) %>% 
								select(-c(Description, leading_edge, core_enrichment)) %>% as.data.frame()
...
"TRAVAGLINI_LUNG_TREM2_DENDRITIC_CELL"
			...
			for (i in 1:length(soi)){
				...
				# description
					set_description <- strwrap(as.character((all_gene_sets %>% filter(gs_name %in% gseaResToUse$ID[i]) %>% select(gs_description))[1,]), width = 100, simplify = FALSE)
				...
				# heatmap
					...
					core_rowname <- gene_info %>% filter(Entrez %in% core_ID) %>% select(ID)
					# Get expression data
